...
var ui = {
    ...
    button: function ($node, options) {
        // Ensure $node is a jQuery object
        $node = $($node);

        // Create a button element directly as a jQuery object
        const $button = $('<button type="button" class="note-btn btn btn-light btn-sm" role="button" tabindex="-1">Click Me</button>');

        // Apply the tooltip options if they exist
        ...
        // Append the jQuery button object to the $node
        if ($node && $node.length) {
            $node.append($button);
        } else {
            console.error('$node is not defined or is not a valid jQuery object');
        }
